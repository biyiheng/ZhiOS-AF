 ...
 * 模块说明（维护入口）
 * -----------------------------------------------------------------------------
 * 职责     ：把跨平台 RTOS 抽象层映射到 FreeRTOS；换内核仅替换本文件。
 * 依赖     ：FreeRTOS 内核 + 各平台 FreeRTOSConfig.h（bsp/<board>/）。
 * 被谁调用 ：上层所有通过 zhios_rtos.h 使用内核服务的模块。
 * 实时性    ：上下文切换由 FreeRTOS 的 PendSV 汇编端口完成（保存 R4-R11、LR、
 *             PSP/MSP，见 bsp/stm32h743/port_context_switch.s 参考模板），
 *             对应《33-操作系统技术指标体系设计文档》4.1/4.3。
 * 待落地    ：zhio_watchdog_* 为软实现，生产接入硬件独立看门狗（IWDG/WWDG）；
 *             zhio_malloc 依赖 configTOTAL_HEAP_SIZE 堆，硬实时关键路径请改用静态内存。
 * =============================================================================
 */
#include <stdint.h>
#include <stddef.h>
