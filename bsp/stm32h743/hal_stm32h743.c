 ...
 * 模块说明（维护入口）
 * -----------------------------------------------------------------------------
 * 职责     ：STM32H743 外设驱动占位（可替换模块，换板只改本目录）。
 * 依赖     ：hal.h、zhios_rtos；生产依赖 STM32H7 HAL 库与 CubeMX 配置。
 * 被谁调用 ：上层所有 HAL 调用。
 * 驱动指标 ：对应《33-操作系统技术指标体系设计文档》4.4"设备驱动"维度。生产实现须：
 *             1) 外设寄存器访问一律使用 volatile 指针，避免编译器重排；
 *             2) 多字节共享数据 / 跨 ISR 数据在访问前后加 DMB/DSB/ISB 内存屏障；
 *             3) 中断底半部经汇编级原子位图或事件标志组异步触发（见 port_context_switch.s）；
 *             4) DMA 采用零拷贝环形缓冲区；4.5"功耗管理"WFI/SLEEPDEEP 见 lowpower_asm.h。
 * 待落地    ：本文件当前为 (void) 占位，寄存器级实现由项目按 CubeMX 生成后填充。
 * =============================================================================
 */
#include <string.h>
#include <stdint.h>
