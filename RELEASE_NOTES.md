...
- **CI 结果**：`build-test` 任务（构建 / ZTEST 单元测试 / cppcheck / Python 仿真 / Agent E2E / 可部署性自检）全部通过；`arm-verify` 链接验证通过，QEMU 压测为冒烟测试（见已知限制）。
- **分支合并**：`release` 完整快照已合并至 `main`（统一采用 Apache-2.0 许可），main 与 release 内容一致。
...
| GitHub Actions `arm-verify` | M7/M3 链接验证通过；QEMU Jitter 压测以冒烟阈值验证（见已知限制） |
