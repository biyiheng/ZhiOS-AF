 ...
 * 模块说明（维护入口）
 * -----------------------------------------------------------------------------
 * 职责     ：Agent 间消息路由（单播/广播/接收），data 零拷贝共享指针。
 * 依赖     ：message_bus.h、zhios_rtos（zhio_queue_* FIFO 队列）。
 * 被谁调用 ：auto_agent、sub_agents、capability、外部 include/message_bus.h。
 * 内存     ：消息 data 为调用方所有，总线不复制/不释放，避免热路径拷贝；
 *             对应《33》4.2 内存管理（零拷贝）维度。
 * 可替换   ：优先级抢占式排队列为可替换增强（设计文档已给方案）。
 * =============================================================================
 */
#include <string.h>
#include <stdint.h>
